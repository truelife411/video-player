...
- **标签系统** ⭐
  - 快速内容指纹绑定：不超过 128MB 的文件计算完整 SHA256，更大的文件使用文件大小 + 头尾各 64MB SHA256
  - 文件改名或移动后重新打开，可沿用原标签与播放进度；移动盘或网络路径暂时离线时，搜索会保留数据库记录
  - 预置标签：星级（1–7★）、画质（480p/720p/1080p/4K）
  - 自定义标签支持单值/多值、枚举/自由文本，并可修改显示名称
  - 双击画面切换全屏；`T` 键或右键唤出磨砂玻璃标签卡片
...
- **标签搜索**
  - `Ctrl+F` 唤出磨砂玻璃搜索浮层
  - 关键词与星级可组合筛选，搜索结果由后端排序和分页
  - 实时筛选（按文件名或任意标签值）
  - 单击播放 / 右键"在文件夹中显示"

- **系统集成**
  - MSI / NSIS 安装器会注册到 Windows“打开方式”，并在卸载时清理关联；受 UserChoice 限制，首次仍需手动选择“始终使用此应用”
  - 单实例运行可在设置中开关，修改后下次启动生效
  - 轻量容器探测用于启动前调整窗口；探测失败会自动回退到 libmpv，不影响正常播放
