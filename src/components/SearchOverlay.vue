...
const totalPages = computed(() => Math.max(1, props.totalPages ?? 0));
const pagedResults = computed(() => props.results);
const effectiveSortKey = computed(() => props.sortKey ?? sortKey.value);
const effectiveSortDir = computed(() => props.sortDir ?? sortDir.value);

function goToPage(p: number) {
  emit("changePage", Math.max(1, Math.min(totalPages.value, p)));
}
  ...
  <div class="backdrop" @click="emit('close')">
    <div class="overlay" @click.stop @wheel.stop>
      ...
      <div class="search-bar">
        ...
        <span class="count" v-if="hasFilter && !searching">{{ total ?? results.length }} 个结果</span>
      ...
      <div class="table-wrap">
        <table class="table">
          <thead>
            <tr>
              <th
                ...
                :key="col.key"
                :class="[col.cls, { sortable: true, active: effectiveSortKey === col.key }]"
                @click="toggleSort(col.key)"
              >
                {{ col.label }}
                <span class="sort-arrow" v-if="effectiveSortKey === col.key">{{ effectiveSortDir === "asc" ? "▲" : "▼" }}</span>
              </th>
          ...
          <tbody>
            ...
            <tr v-else-if="error">
              <td colspan="7" class="empty-row error-row">搜索失败：{{ error }}</td>
            </tr>
      ...
      <div class="pager pager-bottom" v-if="totalPages > 1">
        <span class="page-info">第 {{ currentPage }} / {{ totalPages }} 页（共 {{ total ?? results.length }} 条）</span>
