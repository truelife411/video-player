...
const bufferedRatio = computed(() => {
  if (props.duration <= 0 || !props.bufferedUntil) return 0;
  return Math.min(1, Math.max(displayRatio.value, props.bufferedUntil / props.duration));
});
const abA = computed(() => props.duration > 0 && props.abLoopA != null ? Math.min(1, Math.max(0, props.abLoopA / props.duration)) : null);
const abB = computed(() => props.duration > 0 && props.abLoopB != null ? Math.min(1, Math.max(0, props.abLoopB / props.duration)) : null);
...
<template>
  <div class="progress" :class="{ disabled }">
    ...
    >
      ...
      <div class="buffered" :style="{ width: bufferedRatio * 100 + '%' }"></div>
      <div
        v-if="abA !== null && abB !== null && abB > abA"
        class="ab-range"
        :style="{ left: abA * 100 + '%', width: (abB - abA) * 100 + '%' }"
      ></div>
      <div v-if="abA !== null" class="ab-mark" :style="{ left: abA * 100 + '%' }"></div>
      <div v-if="abB !== null" class="ab-mark" :style="{ left: abB * 100 + '%' }"></div>
      <!-- 已播放 -->
