  ...
  <div class="panel" @click.stop @dblclick.stop @wheel.stop>
    ...
    <div class="panel-body">
      ...
      <section class="card">
        ...
        <div class="btn-row">
          <button class="pill" :disabled="abLoopBusy" @click="emit('setAbA')">
            A 点<span v-if="abLoopA !== null" class="val">{{ formatTime(abLoopA) }}</span>
          </button>
          <button class="pill" :disabled="abLoopBusy || abLoopA === null" @click="emit('setAbB')">
            B 点<span v-if="abLoopB !== null" class="val">{{ formatTime(abLoopB) }}</span>
          </button>
          <button v-if="abLoopA !== null" class="pill" :disabled="abLoopBusy" @click="emit('seekAbA')">跳到 A</button>
          <button class="pill warn" :disabled="abLoopBusy" @click="emit('clearAb')">清除</button>
        </div>
      </section>
      ...
      <section class="card">
        ...
        <div class="btn-row">
          <button class="pill" :disabled="screenshotBusy" @click="emit('screenshot', true)">含字幕</button>
          <button class="pill" :disabled="screenshotBusy" @click="emit('screenshot', false)">仅画面</button>
        </div>
      </section>
      ...
      <section class="card">
        <div class="card-label-row">
          ...
          <button class="link-btn" :disabled="transformBusy" @click="emit('resetTransform')" title="还原全部">还原</button>
        </div>
        <p v-if="hFlipped || vFlipped" class="decode-warning">翻转期间使用软件解码，CPU 占用可能升高</p>
        <div class="btn-row">
          <button class="pill" :disabled="transformBusy" :class="{ on: videoRotate % 360 !== 0 }" @click="emit('rotateCcw')" title="逆时针旋转 90°">
            <span class="pill-icon">↺</span>
            <span>逆时针</span>
          </button>
          <button class="pill" :disabled="transformBusy" :class="{ on: videoRotate % 360 !== 0 }" @click="emit('rotateCw')" title="顺时针旋转 90°">
            <span class="pill-icon">↻</span>
            <span>顺时针</span>
          </button>
          <button class="pill" :disabled="transformBusy" :class="{ on: hFlipped }" @click="emit('toggleHFlip')" title="水平翻转">
            <span>水平翻转</span>
          </button>
          <button class="pill" :disabled="transformBusy" :class="{ on: vFlipped }" @click="emit('toggleVFlip')" title="垂直翻转">
            <span>垂直翻转</span>
          </button>
        </div>
      </section>
...
.decode-warning { margin: 0 0 8px; color: #ffbf69; font-size: 12px; }
