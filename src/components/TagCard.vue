...
const props = defineProps<{
  ...
  getValues: (typeId: number) => string[];
  hash: string;
}>();
...
const emit = defineEmits<{
  ...
  setValues: [typeId: number, values: string[]];
}>();

// 星级专用渲染
function isStar(t: TagType): boolean {
  return t.system_key === "stars";
}
  ...
  <div class="card" @click.stop @dblclick.stop @wheel.stop @contextmenu.prevent.stop="emit('close')">
    ...
    <div class="body">
      ...
      <div v-for="t in tagTypes" :key="t.id" class="row">
        ...
        <!-- 枚举型（下拉） -->
        <div v-else-if="t.value_type === 'enum' && t.is_multi" class="multi-options">
          <button
            v-for="opt in t.options"
            :key="opt.id"
            class="option-chip"
            :class="{ on: getValues(t.id).includes(opt.value) }"
            @click="emit('setValues', t.id, getValues(t.id).includes(opt.value) ? getValues(t.id).filter(v => v !== opt.value) : [...getValues(t.id), opt.value])"
          >{{ opt.value }}</button>
        </div>
        ...
        >
          ...
          <option v-for="opt in t.options" :key="opt.id" :value="opt.value">{{ opt.value }}</option>
        </select>
...
<style scoped>
.multi-options { display:flex; flex-wrap:wrap; gap:6px; }
.option-chip { border:1px solid rgba(255,255,255,.15); background:rgba(255,255,255,.06); color:#ddd; border-radius:12px; padding:4px 9px; cursor:pointer; }
.option-chip.on { background:#367bf5; color:white; border-color:#367bf5; }
