...
import { resolveShortcut, shortcutContext } from "./utils/shortcuts";
...
// 后端 pending-open 信箱保证冷启动阶段的第二实例请求不会丢失。
async function consumePendingOpen() {
  const path = await invoke<string | null>("take_pending_open_file").catch(() => null);
  if (path) await openFileWhenReady(path);
}
...
async function openFileWhenReady(path: string) {
  ...
  while ((!mpv.isReady.value || !mpv.settingsReady.value) && Date.now() < deadline) {
...
function toggleSearch() {
  ...
  if (willOpen && search.hasFilter()) search.refresh();
}
...
function onKeyDown(e: KeyboardEvent) {
  const action = resolveShortcut(e, shortcutContext(e.target, !!mpv.currentFile.value));
  if (!action) return;
  e.preventDefault();
  const run = (promise: Promise<unknown>) => promise.catch((error) => showToast(String(error)));
  switch (action) {
    case "toggle-play": void run(mpv.togglePlay()); break;
    case "seek-forward": void run(mpv.seekBy(mpv.skipSeconds.value)); break;
    case "seek-back": void run(mpv.seekBy(-mpv.skipSeconds.value)); break;
    case "volume-up": void run(mpv.setVolume(Math.min(100, mpv.volume.value + 5))); flashVolume(); break;
    case "volume-down": void run(mpv.setVolume(Math.max(0, mpv.volume.value - 5))); flashVolume(); break;
    case "toggle-mute": void run(mpv.toggleMute()); flashVolume(); break;
    case "toggle-fullscreen": void toggleFullscreen(); break;
    case "toggle-search": void toggleSearch(); break;
    case "toggle-tag": void toggleTagCard(); break;
    case "screenshot": void takeScreenshot(true); break;
    case "toggle-subtitles": {
      const tracks = mpv.subTracks.value;
      const index = tracks.findIndex((track) => track.id === mpv.currentSubId.value);
      const next = index < 0 ? tracks[0]?.id ?? 0 : index + 1 < tracks.length ? tracks[index + 1].id : 0;
      void run(mpv.setSubTrack(next)); break;
    }
    case "frame-back": void run(mpv.frameBackStep()); break;
    case "frame-forward": void run(mpv.frameStep()); break;
    case "rotate-cw": void run(mpv.rotate90()); break;
    case "rotate-ccw": void run(mpv.rotateMinus90()); break;
    case "flip-h": void run(mpv.toggleHFlip()); break;
    case "flip-v": void run(mpv.toggleVFlip()); break;
    case "ab-a": void run(mpv.setAbLoopA()); break;
    case "ab-b": void run(mpv.setAbLoopB()); break;
    case "ab-clear": void run(mpv.clearAbLoop()); break;
    case "escape":
      if (showSettings.value || showPlayback.value || showTagCard.value || showSearch.value) closeAllOverlays();
  ...
  >
    ...
    <Transition name="pop">
      <TagCard
        ...
        :get-values="tags.getValues"
        :hash="videoHash"
        @close="showTagCard = false"
        @set-value="(id, v) => tags.setValue(id, v)"
        @set-values="(id, values) => tags.setValues(id, values)"
      />
    </Transition>
    ...
    <Transition name="fade">
      <div
        v-if="mpv.playbackState.value === 'failed' || mpv.pausedForCache.value || mpv.playbackState.value === 'ended' || mpv.isOpening.value"
        class="opening-hint"
        :class="{ 'error-hint': mpv.playbackState.value === 'failed' }"
        @click.stop
      >
        <template v-if="mpv.playbackState.value === 'failed'">
          <p>{{ mpv.playbackError.value || '无法播放此视频' }}</p>
          <button class="hint-button" @click="mpv.currentFile.value ? mpv.openFile(mpv.currentFile.value) : mpv.openFileDialog()">重试/选择文件</button>
        </template>
        <template v-else-if="mpv.pausedForCache.value">
          <p>缓冲中… {{ Math.round(mpv.cacheBufferingState.value) }}%</p>
        </template>
        <template v-else-if="mpv.playbackState.value === 'ended'">
          <p>播放结束</p>
          <button class="hint-button" @click="mpv.replay()">重新播放</button>
        </template>
        <template v-else>
      ...
      <div v-show="controlsVisible" class="bottom-wrap">
        ...
        <Transition name="pop">
          <SettingsPanel
            ...
            :window-size-policy="mpv.windowSizePolicy.value"
            :resume-mode="mpv.resumeMode.value"
            @close="showSettings = false"
            @set-skip-seconds="(s) => mpv.setSkipSeconds(s)"
            @set-window-scale="(s) => mpv.setWindowScale(s)"
            @set-window-size-policy="(policy) => mpv.setWindowSizePolicy(policy)"
            @set-resume-mode="(m) => mpv.setResumeMode(m)"
            @create-tag-type="(name, vt, opts, multi) => tags.createTagType(name, vt, opts, multi)"
            @update-tag-type="(id, name, multi) => tags.updateTagType(id, name, multi)"
        ...
        <Transition name="pop">
          <PlaybackPanel
            ...
            :ab-loop-busy="mpv.abLoopBusy.value"
            :screenshot-busy="mpv.screenshotBusy.value"
            :transform-busy="mpv.transformBusy.value"
        ...
        <ControlBar
          ...
          :buffered-until="mpv.bufferedUntil.value"
          :ab-loop-a="mpv.abLoopA.value"
          :ab-loop-b="mpv.abLoopB.value"
    ...
    <Transition name="fade">
      <SearchOverlay
        ...
        :sort-key="search.sortKey.value"
        :sort-dir="search.sortDir.value"
        @close="showSearch = false"
        @updatekeyword="(k: string) => search.setKeyword(k)"
        @select-stars="(n: number) => search.setStars(n)"
        @change-page="(page: number) => search.setPage(page)"
        @change-sort="(key) => search.setSort(key)"
